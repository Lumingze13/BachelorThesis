...
/**
 * Human-readable cause for a DB error. Node's net stack throws AggregateError
 * (empty .message, causes in .errors) when every address of a host refuses the
 * connection — without unwrapping it, admin/health surfaces show the useless
...
const LOCAL_HOST_RE = /^(localhost|127\.\d+\.\d+\.\d+|\[?::1\]?)$/;

/** Actionable hint when the connection target can't be right for a deployment. */
export function dbHint() {
  if (!dbEnabled) return 'Set DATABASE_URL (on Railway: add a Postgres service, then reference ${{Postgres.DATABASE_URL}}).';
  if (LOCAL_HOST_RE.test(hostOf(url))) {
    return 'DATABASE_URL points at localhost — that only works for local dev. On Railway, set the service variable DATABASE_URL to a reference: ${{Postgres.DATABASE_URL}} (add a Postgres service first if there is none).';
  }
  return null;
}
...
export async function probe() {
  if (!dbEnabled) return { ok: false, reason: 'DATABASE_URL not set', hint: dbHint() };
  try {
    await query('SELECT 1');
    return { ok: true, ssl: chosenSsl, host: hostOf(url) };
  } catch (err) {
    const out = { ok: false, reason: describeDbError(err), host: hostOf(url) };
    const hint = dbHint();
...
// --- app_config key/value store + the shared results-share token -------------
...
/**
 * The effective read-only results-share token.
 *  - If RESULTS_TOKEN is set in the env, that ALWAYS wins (host secrets keep
 *    precedence — see README "secrets belong only in env vars").
 *  - Otherwise, when a DB is available, get-or-create a strong token stored in
 *    app_config so a separate supervisor link exists and is stable across
 *    restarts WITHOUT needing a host env var (and is shared across all admins).
 *  - With no env token and no DB, returns null (callers fall back to ADMIN_TOKEN).
 * Returns { token, source: 'env'|'db'|null }.
 */
export async function getEffectiveResultsToken() {
  if (process.env.RESULTS_TOKEN) return { token: process.env.RESULTS_TOKEN, source: 'env' };
  if (!dbEnabled) return { token: null, source: null };
  try {
    let t = await getConfig('results_token');
    if (!t) {
      const crypto = await import('node:crypto');
      const gen = crypto.randomBytes(18).toString('base64url');
      // DO NOTHING (not DO UPDATE) so a concurrent first-creator's token isn't
      // clobbered; then re-read to return whichever value actually persisted.
      await query(
        `INSERT INTO app_config (key, value) VALUES ('results_token', $1) ON CONFLICT (key) DO NOTHING`,
        [gen]
      );
      t = (await getConfig('results_token')) || gen;
    }
    return { token: t, source: 'db' };
  } catch (e) {
