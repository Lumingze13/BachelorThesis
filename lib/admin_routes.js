...
import { dbEnabled, describeDbError, dbHint, getEffectiveResultsToken } from './db.js';
import {
...
export function mountAdminRoutes(app) {
  ...
  // The read-only results-share token (for /results). Admins can read it so the
  // whole team can share the supervisor link without anyone touching the host's
  // env vars. env RESULTS_TOKEN wins; otherwise it's an auto-generated DB token.
  app.get('/api/admin/results-token', requireAdmin, async (req, res) => {
    try {
      const { token, source } = await getEffectiveResultsToken();
      res.json({ token, source }); // source: 'env' | 'db' | null
    } catch (err) {
      console.error('results-token failed:', err?.message || err);
      res.status(500).json({ error: 'Could not read results token.' });
    }
  });
