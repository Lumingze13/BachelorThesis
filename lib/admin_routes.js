...
export function mountAdminRoutes(app) {
  ...
  // Soft-remove a link/session from the lists — KEEPS all data (just labels it
  // archived). This is what the "delete link" buttons call now.
  app.post('/api/admin/sessions/:id/archive', requireAdmin, requireDb, async (req, res) => {
    try {
      const row = await archiveSession(req.params.id);
      if (!row) return res.status(404).json({ error: 'Not found.' });
      res.json({ ok: true, archived_at: row.archived_at });
    } catch (err) {
      console.error('archive session failed:', err?.message || err);
      res.status(500).json({ error: 'Archive failed.' });
    }
  });

  // Undo an archive — bring the row back into the active lists.
  app.post('/api/admin/sessions/:id/unarchive', requireAdmin, requireDb, async (req, res) => {
    try {
      const row = await unarchiveSession(req.params.id);
      if (!row) return res.status(404).json({ error: 'Not found.' });
      res.json({ ok: true });
    } catch (err) {
      console.error('unarchive session failed:', err?.message || err);
      res.status(500).json({ error: 'Restore failed.' });
    }
  });

  // HARD delete — the ONLY path that truly removes data. Deliberately gated to
  // archived rows so participant data can never be removed in one careless step
  // or incidentally: a link must first be archived (labelled), then explicitly
  // purged. Nothing else, anywhere, deletes a session row.
  app.delete('/api/admin/sessions/:id', requireAdmin, requireDb, async (req, res) => {
    try {
      const row = await getSessionRow(req.params.id);
      if (!row) return res.json({ ok: false, notFound: true });
      if (!row.archived_at) {
        return res.status(409).json({
          error: 'Archive this session first. Permanent deletion is only allowed on an already-archived row, so data is never removed by accident.',
        });
      }
