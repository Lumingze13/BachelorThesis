...
/** True once a row holds any participant run data (so it must not be overwritten). */
export function sessionHasRunData(row) {
  if (!row) return false;
  if (row.status && row.status !== 'started') return true;
  for (const col of ['profile', 'pre_survey', 'scores', 'phase_b', 'phase_c', 'post_survey', 'free_continuation']) {
    const v = row[col];
...
/**
 * Decide which row a FRESH participant run should write to for a given link.
 * A participant link pins one session id; reopening it must never clobber an
 * earlier run. If the linked row is still pristine (admin just minted it, no run
 * yet) we use it. If it already holds a run, we FORK a new sibling row that
 * shares the link's identity (pid / study / condition / rec / recruiter), so the
 * earlier run is preserved and both records live under the same link.
 * Returns { id, forked }. We keep as much data as possible — nothing is deleted.
 */
export async function claimSessionForRun(id, fallbackMeta = {}) {
  const row = await getSessionRow(id);
  if (!row) {
    // Link points at an unknown/deleted row — start a fresh run from the meta.
    const created = await createSession(fallbackMeta);
    return { id: created.id, forked: false, created: true };
  }
  if (!sessionHasRunData(row)) return { id: row.id, forked: false };
  const fork = await createSession({
    condition: row.condition, rec: row.rec, study: row.study,
    pid: row.pid, recruiter: row.recruiter,
  });
...
export async function listSessions({ condition, status, recruiter, archived, limit = 200 } = {}) {
  // archived: 'active' (not archived) | 'archived' | 'all' (default — backward compatible).
  const arch = archived === 'active' ? 'null' : archived === 'archived' ? 'notnull' : 'all';
  const { rows } = await query(
    `SELECT id, condition, rec, study, pid, recruiter, status, version, created_at, completed_at, archived_at,
     ...
     WHERE ($1::text IS NULL OR condition = $1)
       ...
       AND ($5::text = 'all' OR ($5::text = 'null' AND archived_at IS NULL) OR ($5::text = 'notnull' AND archived_at IS NOT NULL))
     ORDER BY created_at DESC
     LIMIT $3`,
    [condition || null, status || null, limit, recruiter || null, arch]
  );
  return rows;
}

/** Soft-remove from the admin lists: label the row archived, KEEP all its data.
 *  Idempotent (re-archiving keeps the first timestamp). */
export async function archiveSession(id) {
  const { rows } = await query(
    `UPDATE sessions SET archived_at = COALESCE(archived_at, now()) WHERE id = $1 RETURNING *`, [id]);
  return rows[0] || null;
}

/** Undo an archive — bring the row back into the active lists, data untouched. */
export async function unarchiveSession(id) {
  const { rows } = await query(
    `UPDATE sessions SET archived_at = NULL WHERE id = $1 RETURNING *`, [id]);
  return rows[0] || null;
}

/**
 * HARD delete — the ONLY path that truly removes a session and its messages
 * (FK cascade). Reserved for the explicit "Delete data permanently" action; the
 * admin route additionally gates it to archived rows so data is never removed
 * incidentally.
 */
