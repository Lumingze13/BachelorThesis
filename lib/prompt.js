...
// --- "Day in the life" video scene prompts (optional pre-chat stimulus) --------
// Turns the chosen career + what they told the guide into three short, vivid
// first-person POV scene prompts (morning / afternoon / evening) for a text-to-
// video model. Claude writes them; the video module wraps each in shot/style
// scaffolding. Returns { system, user } for a single completion that must reply
// with ONLY a JSON object: {"morning":"…","afternoon":"…","evening":"…"}.
export function buildDayInLifeScenePrompt(profileData = {}, phaseBNotes = '', location = '') {
  const career = (profileData.career || 'this career').toString().trim();
  const place = (location || '').toString().trim() || NO_LOCATION;
  const carryOver = (phaseBNotes || '').trim();
  const t = timeAnchor();
  const system = `You write prompts for a text-to-video model. From a person's profile you produce three short first-person POV scenes from one ordinary weekday about ten years from now, in a life built around a given career. The scenes are MORNING, AFTERNOON, and EVENING of the same day, same person, same world — so they read as one continuous day.

Hard rules for every scene:
- First-person POV ("camera as the person's eyes"); the viewer never sees the person's face — they ARE them. Hands, desk, street, screens, people across from them are fine.
- Concrete and sensory: a real place, the light at that hour, one or two specific objects or people, what the work physically looks like. No captions, no text overlays, no on-screen words.
- Grounded near-future realism for ${t.futureYear} (subtle, not sci-fi). Plausible for this career in this place; if the place does not fit the career, set it somewhere believable and don't force it.
- 2-4 sentences each, present tense, cinematic but plain. No salaries, no narration, no advice.
- Keep the same apparent person, home, and city across all three.`;
  const user = `Career they are stepping into: ${career}
Place they are drawn to: ${place}
${formatProfile(profileData)}${carryOver ? `\nWhat they told the recommendation guide:\n${carryOver}` : ''}

Write the three POV scenes for an ordinary weekday ten years on. Reply with ONLY a JSON object, no prose, no code fence:
