...
import {
  createSession, getSessionRow, saveSession, reconstructStudy, claimSessionForRun,
} from './sessions.js';
...
export function mountStudyRoutes(app) {
  ...
  // Claim a (pinned participant-link) session for a FRESH run. Returns the same
  // id if its row is still pristine, or a new forked sibling id if the link was
  // already used — so reopening a link never overwrites the earlier run. Both
  // records keep the same pid (we preserve data, never overwrite it).
  app.post('/api/sessions/:id/claim', async (req, res) => {
    if (!dbEnabled) return res.json({ id: req.params.id, forked: false, persisted: false });
    try {
      const b = req.body || {};
      const meta = { condition: b.condition, rec: b.rec, study: b.study, pid: b.pid };
      const out = await claimSessionForRun(req.params.id, meta);
      res.json({ ...out, persisted: true });
    } catch (err) {
      console.error('POST /api/sessions/:id/claim failed:', err?.message || err);
      res.status(500).json({ error: 'Could not claim session.' });
    }
  });
