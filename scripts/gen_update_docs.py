...
UX_POLISH = [
    ...
    "Prompt-behaviour QA: test/prompt_behavior_check.mjs runs the real Stage-C prompt through a scripted conversation and OBJECTIVELY measures length-variation + future-grounding across career types (analytical/clinical/caring); both PASS (see docs/prompt_behavior_evidence.md). Re-run with the study's gpt-5.1 (LLM_BASE_URL+UVA_API_TOKEN) before fielding; it exits non-zero on regression.",
